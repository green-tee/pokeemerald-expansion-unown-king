...
// Movement config
#define OW_RUNNING_INDOORS          GEN_5  // In Gen4+, players are allowed to run indoors.
...
// PC settings
#define OW_PC_PRESS_B               GEN_5 // In Gen4, pressing B when holding a Pokémon is equivalent to placing it. In Gen3, it gives the "You're holding a Pokémon!" error.
...
#define OW_PC_MOVE_ORDER            GEN_5 // Starting in Gen4, the order of options in the PC menu change.
...
#define OW_SUPER_LUCK               GEN_5 // In Gen8+, if a Pokémon with Super Luck is leading the party, the wild held item rate is increased to 60%/20%.
#define OW_INFILTRATOR              GEN_5 // In Gen8+, if a Pokémon with Infiltrator is leading the party, the wild encounter rate is halved.
#define OW_HARVEST                  GEN_5 // In Gen8+, if a Pokémon with Harvest is leading the party, there is a 50% chance to encounter a Grass-type Pokémon.
#define OW_LIGHTNING_ROD            GEN_5 // In Gen8+, if a Pokémon with Lightning Rod is leading the party, there is a 50% chance to encounter an Electric-type Pokémon.
#define OW_STORM_DRAIN              GEN_5 // In Gen8+, if a Pokémon with Storm Drain is leading the party, there is a 50% chance to encounter a Water-type Pokémon.
#define OW_FLASH_FIRE               GEN_5 // In Gen8+, if a Pokémon with Flash Fire is leading the party, there is a 50% chance to encounter a Fire-type Pokémon.
...
#define OW_ALTERED_TIME_RATIO           GEN_5        // In GEN_8_PLA, the time in game moves forward 60 seconds for every second in the RTC. In GEN_9, it is 20 seconds. TIME_DEBUG is 1:1, and meant for debugging purposes. This has no effect if OW_USE_FAKE_RTC is FALSE.
...
// Pokémon Center
#define OW_IGNORE_EGGS_ON_HEAL           GEN_5              // In Gen 4+, the nurse in the Pokémon Center does not heal Eggs on healing machine.
