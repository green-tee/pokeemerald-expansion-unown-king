...
void ItemUseInBattle_BagMenu(u8 taskId)
{
    ...
    {
        PlaySE(SE_SELECT);
        if (!GetItemImportance(gSpecialVar_ItemId) && !(B_TRY_CATCH_TRAINER_BALL >= GEN_4 && (GetItemBattleUsage(gSpecialVar_ItemId) == EFFECT_ITEM_THROW_BALL) && (gBattleTypeFlags & (BATTLE_TYPE_TRAINER | BATTLE_TYPE_BOSS))))
            RemoveUsedItem();
